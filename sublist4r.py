...
def banner():
    result = pyfiglet.figlet_format("Sublist4R")
    author = 'Coded By Ahmed Aboul-Ela AND fixed by Anux\n'
    print(result+author)
...
class GoogleEnum(enumratorBaseThreaded):
    ...
    def check_response_errors(self, resp):
        if (type(resp) is str or type(resp) == unicode) and 'Our systems have detected unusual traffic' in resp:
...
def main(domain, threads, savefile, ports, silent, verbose, enable_bruteforce, engines):
    ...
    if engines is None:
        chosenEnums = [
            ...
            CrtSearch, PassiveDNS, GoogleEnum
        ]
    else:
